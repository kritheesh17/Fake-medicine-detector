...
function AppContent() {
  ...
  // Shared by both the popup flow and the redirect flow: makes sure a Firestore
  // profile exists for the signed-in Google user, then routes them in.
  const completeGoogleSignIn = async (firebaseUser) => {
    if (!firebaseUser) {
      return;
    }
  ...
  // On mount, check whether we're returning from a signInWithRedirect() call.
  // Without this, users who fell back to the redirect flow (popup blocked, etc.)
  // would come back signed in but with no Firestore profile ever created.
  useEffect(() => {
    let cancelled = false;
    ...
    const checkRedirectResult = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setError("We could not finish signing in with Google.");
        }
      }
    };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      try {
        ...
        // Only fall back to redirect when the popup itself couldn't be used
        // (blocked, unsupported, or the user closed it) — not for every error.
        const popupUnavailableCodes = [
          "auth/popup-blocked",
          "auth/operation-not-supported-in-this-environment",
          "auth/cancelled-popup-request",
        ];
        if (popupUnavailableCodes.includes(popupError.code)) {
          await signInWithRedirect(auth, provider);
          return; // page will navigate away; redirect result is handled on reload
        }
        if (popupError.code === "auth/popup-closed-by-user") {
          // User cancelled on purpose — not an error worth reporting or redirecting for.
          return;
        }
        throw popupError;
      }
